...
def blending_example1():
    mask_helper = read_image(relpath("externals/mask.jpg"),
                             REPRESENTATION_GRAY)
    mask = np.zeros(mask_helper.shape).astype(np.bool)
    mask[mask_helper == 1] = 1
    mask = mask.astype(np.bool)
    im1 = read_image(relpath("externals/image1.jpg"), REPRESENTATION_RGB)
    im2 = read_image(relpath("externals/image2.jpg"), REPRESENTATION_RGB)
    return im1, im2, mask, pyramid_blending(im1, im2, mask, 20, 11, 2)


def blending_example2():
    mask_helper = read_image(relpath("externals/mask1.jpg"),
                             REPRESENTATION_GRAY)
    mask = np.zeros(mask_helper.shape).astype(np.bool)
    mask[mask_helper == 1] = 1
    mask = mask.astype(np.bool)
    im2 = read_image(relpath("externals/owl.jpg"), REPRESENTATION_RGB)
    im1 = read_image(relpath("externals/coala.jpg"), REPRESENTATION_RGB)
    return im1, im2, mask, pyramid_blending(im1, im2, mask, 20, 11, 3)

#
# if __name__ == '__main__':
#     display_img(blending_example2()[3])
