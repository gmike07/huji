...
class Prover:
    ...
    def add_universal_instantiation(self, instantiation: Union[Formula, str],
        ...
        new_var = list(
            quantified.predicate.free_variables() - quantified.free_variables())[
            0]
        helper1 = quantified.predicate.substitute({new_var: Term('_')}, set())
        new_line = Formula('->', self._lines[line_number].formula, instantiation)
        step1 = self.add_instantiated_assumption(new_line,
                                                 Prover.UI, {'R': helper1,
                                                             'c': term,
                                                             'x': new_var})
    ...
    def add_tautological_implication(self, implication: Union[Formula, str],
        ...
        formula = Formula('->', self._lines[line_numbers[0]].formula,
                          implication)
        for i in range(len(line_numbers) - 1, -1, -1):
            formula = Formula('->', self._lines[line_numbers[i]].formula,
                              formula)
        ...
        return self.add_mp(implication, line_numbers[0], step)
    ...
    def add_existential_derivation(self, consequent: Union[Formula, str],
        ...
        helper = Formula('A', variable, conditional)
        step1 = self.add_ug(helper, line_number2)
        helper1 = Formula('&', helper, quantified)
        replacement = conditional.first.substitute({variable: Term('_')},
                                                   set())
        step2 = self.add_instantiated_assumption(Formula('->', helper1,
                                                         consequent),
                                                 Prover.ES, {'R': replacement,
                                                             'Q': consequent,
                                                             'x': variable})
        return self.add_tautological_implication(consequent,
                                                 {line_number1, step1, step2})
    ...
    def add_flipped_equality(self, flipped: Union[Formula, str],
        ...
        helper1 = Formula('=', [flipped.arguments[1], flipped.arguments[1]])
        step1 = self.add_instantiated_assumption(helper1, Prover.RX,
                                                 {'c': flipped.arguments[1]})

        helper2 = Formula('=', [flipped.arguments[0], flipped.arguments[1]])
        helper3 = Formula('->', helper1, helper2)
        helper4 = Formula('->', equality, helper3)
        R_replacement = Formula('=', [Term('_'), flipped.arguments[1]])

        step2 = self.add_instantiated_assumption(helper4, Prover.ME,
                                                 {'c': flipped.arguments[1],
                                                  'd': flipped.arguments[0],
                                                  'R': R_replacement})
        step3 = self.add_mp(helper3, line_number, step2)
        step4 = self.add_mp(helper2, step1, step3)
        return step4
    ...
    def add_free_instantiation(self, instantiation: Union[Formula, str],
        ...
        dct = {key: next(fresh_variable_name_generator) for key in
               substitution_map.keys()}
        step2 = line_number
        for key in substitution_map.keys():
            formula = self._lines[step2].formula
            step1 = self.add_ug(Formula('A', key, formula), step2)
            new_formula = formula.substitute({key: Term(dct[key])})
            step2 = self.add_universal_instantiation(new_formula, step1,
                                                     Term(dct[key]))
        for key in substitution_map.keys():
            formula = self._lines[step2].formula
            step1 = self.add_ug(Formula('A', dct[key], formula), step2)
            new_formula = formula.substitute({dct[key]: substitution_map[key]})
            step2 = self.add_universal_instantiation(new_formula, step1,
                                                     substitution_map[key])
        return step2
    ...
    def add_substituted_equality(self, substituted: Union[Formula, str],
        ...
        # instantiation: Union[Formula, str],  line_number: int,
        # substitution_map: Mapping[str, Union[Term, str]]
        term = substituted.arguments[0]
        mapping = {'c': equality.arguments[0], 'd': equality.arguments[1],
                   'R': Formula('=', [term, parametrized_term])}
        helper1 = Formula('=', [term, term])
        helper2 = Formula('=', [term, substituted.arguments[1]])
        helper3 = Formula('->', helper1, helper2)
        helper4 = Formula('->', equality, helper3)
        step1 = self.add_instantiated_assumption(helper4, Prover.ME, mapping)
        step2 = self.add_instantiated_assumption(helper1, Prover.RX,
                                                 {'c': term})
        step3 = self.add_mp(helper3, line_number, step1)
        step4 = self.add_mp(helper2, step2, step3)
        return step4
    ...
    def _add_chaining_of_two_equalities(self, line_number1: int,
        ...
        # helper0 = (a==c)
        helper0 = Formula('=',
                          [equality1.arguments[0], equality2.arguments[1]])
        # helper1 = (a==_)
        helper1 = Formula('=', [equality1.arguments[0], Term('_')])
        # helper2 = (a==b->a==c)
        helper2 = Formula('->', equality1, helper0)
        # helper3 = (b==c)->(a==b->a==c)
        helper3 = Formula('->', equality2, Formula('->', equality1, helper0))
        mapping = {'c': equality2.arguments[0], 'd': equality2.arguments[1],
                   'R': helper1}
        step1 = self.add_instantiated_assumption(helper3, Prover.ME, mapping)
        step2 = self.add_mp(helper2, line_number2, step1)
        return self.add_mp(helper0, line_number1, step2)
