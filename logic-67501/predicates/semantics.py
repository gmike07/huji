...
class Model(Generic[T]):
    ...
    Attributes:
        ...
        relation_arities (`~typing.Mapping`\\[`str`, `int`]): mapping from
            each relation name to the arity of the relation, or to ``-1``
            if the relation is the empty relation.
        relation_meanings (`~typing.Mapping`\\[`str`, `~typing.AbstractSet`
            \\[`~typing.Tuple`\\[`T`, ...]]]):
        ...
        function_meanings (`~typing.Mapping`\\[`str`, `~typing.Mapping`
        \\[`~typing.Tuple`\\[`T`, ...], `T`]]):
            mapping from each n-ary function name to the mapping from each
            argument n-tuple (of universe elements) to the universe element
            that the function outputs given these arguments.
    """
    ...
    def eval_formula_helper(self, formula: Formula, assignment: Dict[str, T]) \
            -> bool:
    ...
    def check_formula_true(self, formula: Formula, free_variables: List[str],
                           assignment: Dict[str, T], i=0) -> bool:
        if len(free_variables) == i:
            return self.evaluate_formula(formula, assignment)
        for option in self.universe:
            assignment[free_variables[i]] = option
            if not self.check_formula_true(formula, free_variables,
                                           assignment, i + 1):
                return False
        return True
    ...
    def is_model_of(self, formulas: AbstractSet[Formula]) -> bool:
        ...
        for formula in formulas:
            ...
            if not self.check_formula_true(formula, free_variables, dict()):
                return False
        return True
