...
class Schema:
    ...
    class BoundVariableError(Exception):
        ...
        def __init__(self, variable_name: str, relation_name: str):
            ...
            Parameters: variable_name: variable name that is to become bound
            in a way that is disallowed during a schema instantiation.
    ...
    def _instantiate_helper(formula: Formula,
        ...
        with the term mapped to this name by this map. 2. Substitute each 
        nullary invocation of each relation name that is a key of the given 
        relations instantiation map with the formula mapped to it by this 
        map. 3. For each unary invocation of each relation name that is a 
        key of the given relations instantiation map, first perform all 
        substitutions to the argument of this invocation (according to the 
        given constants and variables instantiation map), then substitute 
        the result for each occurrence of the constant name ``'_'`` in the 
        formula mapped to the relation name by this map, and then substitute 
        the result for this unary invocation of the relation name. 

        Only names that originate in the given formula are substituted (
        i.e., names originating in one of the above substitutions are not 
        ...
        return Schema._instantiate_helper1(formula,
                                           constants_and_variables_instantiation_map,
                                           relations_instantiation_map,
                                           bound_variables)

    @staticmethod
    def _instantiate_helper1(formula: Formula, con_var_map: Mapping[str, Term],
                             rel_map: Mapping[str, Formula],
                             bound_variables: AbstractSet[str] = frozenset()) \
            -> Formula:
        if is_unary(formula.root):
            first = Schema._instantiate_helper1(formula.first, con_var_map,
                                                rel_map, bound_variables)
            return Formula(formula.root, first)
        if is_binary(formula.root):
            first = Schema._instantiate_helper1(formula.first, con_var_map,
                                                rel_map, bound_variables)
            second = Schema._instantiate_helper1(formula.second, con_var_map,
                                                 rel_map, bound_variables)
            return Formula(formula.root, first, second)
        if is_quantifier(formula.root):
            new_var = formula.variable
            if new_var in con_var_map:
                new_var = con_var_map[new_var].root
            new_bound_variables = bound_variables | {new_var}
            predicate = Schema._instantiate_helper1(formula.predicate,
                                                    con_var_map, rel_map,
                                                    new_bound_variables)
            return Formula(formula.root, new_var, predicate)
        if is_relation(formula.root) or is_equality(formula.root):
            try:
                new_formula = Schema.handle_relation(formula, con_var_map,
                                                     rel_map, bound_variables)
                return new_formula
            except ForbiddenVariableError as e:
                raise Schema.BoundVariableError(e.variable_name,
                                                formula.root)

    @staticmethod
    def handle_relation(formula: Formula, con_var_map: Mapping[str, Term],
                        rel_map: Mapping[str, Formula],
                        bound_variables: AbstractSet[str] = frozenset()) \
            -> Formula:
        """
        :param formula: the current formula
        :param con_var_map: the constant variable map
        :param rel_map: the relation map
        :param bound_variables: the bound variables
        :return: the new formula from replacing the relation
        """
        if formula.root not in rel_map:
            return formula.substitute(con_var_map, set())
        if len(formula.arguments) == 0:
            Schema.raise_exception_if_bound(rel_map[formula.root],
                                            bound_variables)
            return rel_map[formula.root]

        term = formula.arguments[0].substitute(con_var_map, set())
        mapper = {'_': term}
        Schema.raise_exception_if_bound(rel_map[formula.root], bound_variables)
        new_formula = rel_map[formula.root].substitute(mapper, set())
        return new_formula

    @staticmethod
    def raise_exception_if_bound(formula, bound_variables):
        """
        :param formula: the new formula
        :param bound_variables: the BAD variables
        :raises an error if the formula uses bound_variables
        """
        variables = formula.free_variables() & bound_variables
        if variables != set():
            raise ForbiddenVariableError(list(variables)[0])
    ...
    def instantiate(self, instantiation_map: InstantiationMap) -> \
        ...
        Returns:
            ...
            with the term to which that template constant name is mapped. 2.
            Each occurrence in the formula of the current schema of each
            ...
            mapped. 3. Each nullary invocation in the formula of the current
            schema of each template relation name specified in the given map
            is substituted with the formula to which that template relation
            name is mapped. 4. Each unary invocation in the formula of the
            current schema of each template relation name specified in the
            given map is substituted with the formula to which that template
            relation name is mapped, in which each occurrence of the
            constant name ``'_'`` is substituted with  the instantiated
            argument of that invocation of the template relation name (that
            is, the term that results from instantiating the argument of
            that invocation by performing all the specified substitutions on
            it).
            ...
            1. A free occurrence of a variable name in the formula
            substituted for the template relation name becomes bound by a
            quantification in the instantiated schema formula, except if the
            template relation name is unary and this free occurrence
            originates in the instantiated argument of the relation
            invocation. 2. For a unary invocation: a variable name in the
            instantiated argument of that invocation becomes bound by a
            quantification in the formula that is substituted for the
            invocation of the template relation name.
        ...
        Examples:
            ...
            instantiated argument ``'f(plus(a,y))'`` of the second
            invocation of ``'Q'`` is to become bound by the quantification
            in the formula substituted for ``'Q'``:
        ...
        for key in instantiation_map:
            if key not in self.templates:
                return None
            if is_variable(key):
                con_var_dct[key] = Term(instantiation_map[key])
            if is_constant(key):
                con_var_dct[key] = instantiation_map[key]
            if is_relation(key):
                rel_dct[key] = instantiation_map[key]
        try:
            return Schema._instantiate_helper(self.formula, con_var_dct,
                                              rel_dct, bound_vars)
...
class Proof:
    ...
    class AssumptionLine:
        ...
        Attributes:
            formula (`~predicates.syntax.Formula`): the formula justified by
                the line.
            assumption (`Schema`): the assumption/axiom that instantiates the
                formula.
            instantiation_map (`~typing.Mapping`\\[`str`,
            `~typing.Union`\\[`~predicates.syntax.Term`, `str`,
            `~predicates.syntax.Formula`]]):
        ...
        def __init__(self, formula: Formula, assumption: Schema,
            ...
            """Initializes an `~Proof.AssumptionLine` from its formula,
            its justifying assumption, and its instantiation map from the
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            """Checks if the current line is validly justified in the context
             of the specified proof.

            Parameters: assumptions: assumptions/axioms of the proof. lines:
            lines of the proof. line_number: line number of the current line
            in the given lines.
            ...
            if self.assumption not in assumptions:
                return False
            if Schema.instantiate(self.assumption, self.instantiation_map) \
                    != self.formula:
                return False
            return True
    ...
    class MPLine:
        ...
        Attributes: formula (`~predicates.syntax.Formula`): the formula
        justified by the line. antecedent_line_number (`int`): the line
        number of the antecedent of the MP inference justifying the line.
        ...
        def __init__(self, formula: Formula, antecedent_line_number: int,
            ...
            """Initializes a `~Proof.MPLine` from its formula and line
            numbers of the antecedent and conditional of the MP inference
            justifying it.

            Parameters: formula: the formula to be justified by the line.
            antecedent_line_number: the line number of the antecedent of the
            MP inference justifying the line. conditional_line_number: the
            line number of the conditional of the MP inference justifying
            the line.
            """
            self.formula = formula
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            if self.antecedent_line_number >= line_number \
                ...
                return False
            line1 = lines[self.antecedent_line_number].formula
            line2 = lines[self.conditional_line_number].formula
            return line2 == Formula('->', line1, self.formula)
    ...
    class UGLine:
        ...
        Attributes: formula (`~predicates.syntax.Formula`): the formula
        justified by the line. predicate_line_number (`int`): the line
        number of the predicate of the UG inference justifying the line.
        """
        formula: Formula
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            Returns: ``True`` if the formula of the current line is of the
            form ``'A``\\ `x`\ ``[``\ `predicate`\ ``]'``, where `predicate`
            is the formula of the line from the given lines whose number is
            the predicate line number justifying the current line and `x` is
            any variable name, ``False`` otherwise.
            """
            assert line_number < len(lines) and lines[line_number] is self
            ...
            if self.predicate_line_number >= line_number:
                return False
            return self.formula.predicate == \
                   lines[self.predicate_line_number].formula
    ...
    class TautologyLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            """
            assert line_number < len(lines) and lines[line_number] is self
            formula, mapper = self.formula.propositional_skeleton()
...
def axiom_specialization_map_to_schema_instantiation_map(
    ...
    for key in propositional_specialization_map.keys():
        formula = propositional_specialization_map[key]
        mapper[key.upper()] = Formula.from_propositional_skeleton(formula,
                                                                  substitution_map)
    return mapper
...
def prove_from_skeleton_proof(formula: Formula,
    ...
    for line in skeleton_proof.lines:
        if line.rule != MP:
            propositional_map = line.rule.specialization_map(
                PropositionalInferenceRule([], line.formula))
            instance_map = axiom_specialization_map_to_schema_instantiation_map(
                propositional_map, substitution_map)
            schema = PROPOSITIONAL_AXIOM_TO_SCHEMA[line.rule]
            new_formula = Schema.instantiate(schema, instance_map)
            proof_lines.append(
                Proof.AssumptionLine(new_formula, schema, instance_map))
        else:
            line1_number = line.assumptions[0]
            line2_number = line.assumptions[1]
            new_formula = proof_lines[line2_number].formula.second
            proof_lines.append(
                Proof.MPLine(new_formula, line1_number, line2_number))
    return Proof(PROPOSITIONAL_AXIOMATIC_SYSTEM_SCHEMAS, formula, proof_lines)
...
def prove_tautology(tautology: Formula) -> Proof:
    ...
    formula, mapper = tautology.propositional_skeleton()
    propositional_proof = prove_propositional_tautology(formula)
    return prove_from_skeleton_proof(tautology, propositional_proof, mapper)
