...
int CellularSetupInternetConnectionProfile(int inact_time_sec)
{
    ...
    printf("Cellular: Completed setting internet connection profile\n");
    return SUCCESS_CODE;
}
...
int CellularSetupInternetServiceProfile(char *IP, int port, int keepintvl_sec)
{
    ...
    {
        printf("Cellular: CellularSetupInternetConnectionProfile hasn't been called yet\n");
        return FAILURE_CODE;
    }
    ...
    printf("Cellular: Completed setting internet service profile\n");
...
int CellularConnect()
{
    ...
    printf("Cellular: Setting configuration: tcpWithUrc to off\n");
    if (SetSettingsConfiguration(scfg_tcpWithUrc, "\"off\"") < 0)
    {
        printf("Cellular: Failed turning off URCs\n");
        return FAILURE_CODE;
    }

    if (OpenInternetConnection() < 0)
    {
        printf("Cellular: Failed opening connection\n");
        CellularClose();
        return FAILURE_CODE;
    }
