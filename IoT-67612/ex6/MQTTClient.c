...
static int NetConnect(void *context, const char *host, word16 port, int timeout_ms) {
    printf("MQTTClient: Initializing socket\n");

    SocketContext *sock = (SocketContext *) context;
    sock->host = host;

    if (SocketInit(host, port) < 0) {
        printf("MQTTClient: Failed initializing socket\n");
        return MQTT_CODE_ERROR_SYSTEM;
    }

    printf("MQTTClient: Initialized socket successfully\n");

    printf("MQTTClient: Connecting to the host\n");

    if (SocketConnect() < 0) {
        printf("MQTTClient: Failed connecting to host\n");
        return MQTT_CODE_ERROR_NETWORK;
    }

    printf("MQTTClient: Connected to the host successfully\n");
...
static int NetRead(void *context, byte *buf, int buf_len, int timeout_ms) {
    ...
    if (n == 0) {
        printf("MQTTClient: socket timeout\n");
        return MQTT_CODE_ERROR_TIMEOUT;
    }

    if (n < 0) {
        printf("MQTTClient: Failed reading from socket\n");
        return MQTT_CODE_ERROR_NETWORK;
    }
...
static int NetWrite(void *context, const byte *buf, int buf_len, int timeout_ms) {
    if (SocketWrite(buf, buf_len) < buf_len) {
        printf("MQTTClient: Failed writing to socket\n");
        return MQTT_CODE_ERROR_NETWORK;
    }
...
static int NetDisconnect(void *context) {
    if (SocketClose() < 0) {
        printf("MQTTClient: Failed closing socket\n");
        return MQTT_CODE_ERROR_SYSTEM;
    }
