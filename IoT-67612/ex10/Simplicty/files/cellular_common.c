...
static char no_carrier_resp[] = "NO CARRIER";
...
void turnOffEcho()
{
    ...
    my_printf("Cellular: Turned off echo\n");
    echo_off = TRUE;
...
{
    if (strstr(buf, no_carrier_resp) == NULL)
    {
        return FALSE;
    }
...
int SendCmdRecvResp(unsigned char *cmd, unsigned char *resp_buf, unsigned int resp_max_size, unsigned int timeout_ms)
{
    ...
    {
        my_printf("Cellular: Failed sending command\n");
        return FAILURE_CODE;
    }
...
int SetSettingsConfiguration(char *config_name, char *config_values)
{
    ...
    {
        my_printf("Cellular: Failed assembling configuration change\n");
        return FAILURE_CODE;
    }
