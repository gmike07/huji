...
sl_status_t sl_iostream_usart_init_gps(void);

sl_status_t sl_iostream_usart_init_vcom(void);


// Instance(s) handle and context variable 
static sl_iostream_uart_t sl_iostream_gps;
sl_iostream_t *sl_iostream_gps_handle = &sl_iostream_gps.stream;
sl_iostream_uart_t *sl_iostream_uart_gps_handle = &sl_iostream_gps;
static sl_iostream_usart_context_t  context_gps;
static uint8_t  rx_buffer_gps[SL_IOSTREAM_USART_GPS_RX_BUFFER_SIZE];
sl_iostream_instance_info_t sl_iostream_instance_gps_info = {
  .handle = &sl_iostream_gps.stream,
  .name = "gps",
  .type = SL_IOSTREAM_TYPE_UART,
  .periph_id = SL_IOSTREAM_USART_GPS_PERIPHERAL_NO,
  .init = sl_iostream_usart_init_gps,
};
...
{
  sl_status_t status;
  USART_InitAsync_TypeDef init_gps = USART_INITASYNC_DEFAULT;
  init_gps.baudrate = SL_IOSTREAM_USART_GPS_BAUDRATE;
  init_gps.parity = SL_IOSTREAM_USART_GPS_PARITY;
  init_gps.stopbits = SL_IOSTREAM_USART_GPS_STOP_BITS;
#if (_SILICON_LABS_32B_SERIES > 0)
#if (SL_IOSTREAM_USART_GPS_FLOW_CONTROL_TYPE != uartFlowControlSoftware)
  init_gps.hwFlowControl = SL_IOSTREAM_USART_GPS_FLOW_CONTROL_TYPE;
#else
  init_gps.hwFlowControl = usartHwFlowControlNone;
#endif
#endif
  sl_iostream_usart_config_t config_gps = { 
    .usart = SL_IOSTREAM_USART_GPS_PERIPHERAL,
    .clock = SL_IOSTREAM_USART_CLOCK_REF(SL_IOSTREAM_USART_GPS_PERIPHERAL_NO),
    .tx_port = SL_IOSTREAM_USART_GPS_TX_PORT,
    .tx_pin = SL_IOSTREAM_USART_GPS_TX_PIN,
    .rx_port = SL_IOSTREAM_USART_GPS_RX_PORT,
    .rx_pin = SL_IOSTREAM_USART_GPS_RX_PIN,
#if (_SILICON_LABS_32B_SERIES > 0)
#if defined(SL_IOSTREAM_USART_GPS_CTS_PORT)
    .cts_port = SL_IOSTREAM_USART_GPS_CTS_PORT,
    .cts_pin = SL_IOSTREAM_USART_GPS_CTS_PIN,
#endif
#if defined(SL_IOSTREAM_USART_GPS_RTS_PORT)
    .rts_port = SL_IOSTREAM_USART_GPS_RTS_PORT,
    .rts_pin = SL_IOSTREAM_USART_GPS_RTS_PIN,
#endif
#endif
#if defined(GPIO_USART_ROUTEEN_TXPEN)
    .usart_index = SL_IOSTREAM_USART_GPS_PERIPHERAL_NO,
#elif defined(USART_ROUTEPEN_RXPEN)
    .usart_tx_location = SL_IOSTREAM_USART_GPS_TX_LOC,
    .usart_rx_location = SL_IOSTREAM_USART_GPS_RX_LOC,
#if defined(SL_IOSTREAM_USART_GPS_CTS_PORT)
    .usart_cts_location = SL_IOSTREAM_USART_GPS_CTS_LOC,
#endif
#if defined(SL_IOSTREAM_USART_GPS_RTS_PORT)
    .usart_rts_location = SL_IOSTREAM_USART_GPS_RTS_LOC,
#endif
#else
    .usart_location = SL_IOSTREAM_USART_GPS_ROUTE_LOC,
#endif
  };
  sl_iostream_uart_config_t uart_config_gps = {
    .tx_irq_number = SL_IOSTREAM_USART_TX_IRQ_NUMBER(SL_IOSTREAM_USART_GPS_PERIPHERAL_NO),
    .rx_irq_number = SL_IOSTREAM_USART_RX_IRQ_NUMBER(SL_IOSTREAM_USART_GPS_PERIPHERAL_NO),
    .rx_buffer = rx_buffer_gps,
    .rx_buffer_length = SL_IOSTREAM_USART_GPS_RX_BUFFER_SIZE,
    .lf_to_crlf = SL_IOSTREAM_USART_GPS_CONVERT_BY_DEFAULT_LF_TO_CRLF,
    .rx_when_sleeping = SL_IOSTREAM_USART_GPS_RESTRICT_ENERGY_MODE_TO_ALLOW_RECEPTION,
#if defined(SL_IOSTREAM_USART_GPS_FLOW_CONTROL_TYPE)
#if (SL_IOSTREAM_USART_GPS_FLOW_CONTROL_TYPE == uartFlowControlSoftware)
  ...
  status = sl_iostream_usart_init(&sl_iostream_gps,
                                  &uart_config_gps,
                                  &init_gps,
                                  &config_gps,
                                  &context_gps);
...
// GPS IRQ Handler
void SL_IOSTREAM_USART_TX_IRQ_HANDLER(SL_IOSTREAM_USART_GPS_PERIPHERAL_NO)(void)
{
  sl_iostream_usart_irq_handler(sl_iostream_gps.stream.context);
}

void SL_IOSTREAM_USART_RX_IRQ_HANDLER(SL_IOSTREAM_USART_GPS_PERIPHERAL_NO)(void)
{
  sl_iostream_usart_irq_handler(sl_iostream_gps.stream.context);
}
...
sl_power_manager_on_isr_exit_t sl_iostream_usart_gps_sleep_on_isr_exit(void)
{
  return sl_iostream_uart_sleep_on_isr_exit(&sl_iostream_gps);
}
                           ...
                           sl_power_manager_em_t to)
{
  ...
  if (((from == SL_POWER_MANAGER_EM2) 
	...
	out = GPIO_PinOutGet(SL_IOSTREAM_USART_GPS_TX_PORT, SL_IOSTREAM_USART_GPS_TX_PIN);
	GPIO_PinModeSet(SL_IOSTREAM_USART_GPS_TX_PORT, SL_IOSTREAM_USART_GPS_TX_PIN, gpioModePushPull, out);
	...
	GPIO_PinModeSet(SL_IOSTREAM_USART_VCOM_TX_PORT, SL_IOSTREAM_USART_VCOM_TX_PIN, gpioModePushPull, out);
      ...
      out = GPIO_PinOutGet(SL_IOSTREAM_USART_GPS_TX_PORT, SL_IOSTREAM_USART_GPS_TX_PIN);
      GPIO_PinModeSet(SL_IOSTREAM_USART_GPS_TX_PORT, SL_IOSTREAM_USART_GPS_TX_PIN, gpioModeDisabled, out);
